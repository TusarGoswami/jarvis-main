...
export const LiveInterviewScreen: React.FC<LiveInterviewScreenProps> = ({
  ...
  const [endingInterview, setEndingInterview] = useState<boolean>(false);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSession((prev: any) => {
          ...
          return {
            ...prev,
            integrity_score: json.integrity_score,
            integrity_events: [...(prev.integrity_events || []), json.event],
          };
        });
      }
    } catch {
      // Background telemetry retry
    }
  }, [interviewId]);
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      if (!isNowFullscreen && !showFullscreenPrompt && session?.status === "in_progress") {
        setFullscreenWarning(true);
        recordIntegritySignal("Full-Screen Exit", 0, "Candidate exited full-screen mode.");
  ...
  }, [showFullscreenPrompt, session?.status, recordIntegritySignal]);

  // 3. Tab-Switch & Window Blur Monitoring
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        blurStartTimeRef.current = Date.now();
      } else if (blurStartTimeRef.current) {
        const elapsedSeconds = (Date.now() - blurStartTimeRef.current) / 1000;
        blurStartTimeRef.current = null;
        if (elapsedSeconds >= 1.0 && session?.status === "in_progress") {
          recordIntegritySignal("Tab Switch", elapsedSeconds, `Candidate switched away for ${Math.round(elapsedSeconds)}s`);
        }
      }
    };
    ...
    const handleWindowFocus = () => {
      if (blurStartTimeRef.current) {
        const elapsedSeconds = (Date.now() - blurStartTimeRef.current) / 1000;
        blurStartTimeRef.current = null;
        if (elapsedSeconds >= 1.5 && session?.status === "in_progress") {
          recordIntegritySignal("Window Focus Lost", elapsedSeconds, `Window lost focus for ${Math.round(elapsedSeconds)}s`);
        }
      }
    };
  ...
  }, [session?.status, recordIntegritySignal]);

  // 4. Synthesize & Speak Question via TTS
  ...
  const fetchSessionState = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (json.data.final_evaluation) {
          setFinalReport(json.data.final_evaluation);
        }
      }
    } catch {
      // Network retry
  ...
  // 6. Server-Synced Countdown Timer
  useEffect(() => {
  ...
  // 7. Real-Time Speech Recognition (STT) Setup
  ...
  // 8. Live Audio Waveform Canvas Animation
  useEffect(() => {
  ...
  // 9. Submit Candidate Answer (Strict 0–10 Evaluation Loop)
  ...
  const handleConfirmEndInterview = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Failed to finalize evaluation.");
    } finally {
  ...
  if (loading && !session) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 font-mono text-cyan-300">
        ...
        <span className="text-sm tracking-wider uppercase">Calibrating Strict Assessment Engine...</span>
      </div>
    );
  }

  // ─── Render Final Results Dashboard if Interview is Complete ───
  if (finalReport || session?.status === "complete" || (session?.current_phase === "complete" && finalReport)) {
    return (
  ...
  return (
    <div className="relative z-10 w-full max-w-7xl mx-auto px-4 py-6 flex flex-col gap-6 font-mono">
        ...
        {showFullscreenPrompt && (
          ...
          >
            <motion.div
              ...
              className="glass-panel p-6 sm:p-8 rounded-3xl max-w-lg w-full border border-cyan-500/50 shadow-[0_0_50px_rgba(0,240,255,0.3)] flex flex-col gap-5 text-center"
            >
              <div className="w-16 h-16 rounded-full bg-cyan-950/80 border border-cyan-500/60 mx-auto flex items-center justify-center text-cyan-400 shadow-[0_0_25px_rgba(0,240,255,0.4)]">
              ...
              <div className="flex flex-col gap-2">
                ...
                <h2 className="text-xl sm:text-2xl font-black text-cyan-200">
                  Full-Screen Mode Required
                </h2>
                <p className="text-xs text-gray-300 font-sans leading-relaxed">
                  To ensure a rigorous and standardized technical evaluation, the assessment runs in full-screen mode.
                </p>
              </div>

              <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800 text-left text-xs font-sans text-gray-300 space-y-2">
                <div className="flex items-center gap-2 text-cyan-300 font-bold">
                  <ShieldAlert className="w-4 h-4 text-cyan-400" />
                  <span>Integrity Guidelines:</span>
                </div>
                <ul className="list-disc list-inside space-y-1 text-gray-400 text-[11px]">
                  <li>Exiting full-screen is monitored and recorded as an integrity signal.</li>
                  <li>Switching tabs or losing window focus is tracked with timestamps.</li>
                  <li>Microphone speech or keyboard responses are evaluated strictly on a 0–10 scale.</li>
                </ul>
              </div>

              <button
                onClick={requestFullscreen}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-black font-bold text-xs uppercase tracking-wider hover:brightness-110 transition shadow-[0_0_20px_rgba(0,240,255,0.4)] flex items-center justify-center gap-2"
              >
                <Maximize2 className="w-4 h-4" />
                <span>Enter Full-Screen &amp; Begin</span>
              </button>
            </motion.div>
          </motion.div>
        )}
      ...
      {/* ─── Full-Screen Exit Warning Overlay ─── */}
      <AnimatePresence>
        {fullscreenWarning && !showFullscreenPrompt && (
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="p-3 bg-red-950/90 border border-red-500/70 rounded-2xl flex items-center justify-between gap-3 text-xs shadow-[0_0_25px_rgba(239,68,68,0.4)]"
          >
            <div className="flex items-center gap-2 text-red-300">
              <AlertTriangle className="w-5 h-5 text-red-400 animate-pulse flex-shrink-0" />
              <span>
                <strong>Integrity Notice:</strong> Full-screen mode was exited. This event has been logged in your assessment record.
              </span>
            </div>
            <button
              onClick={requestFullscreen}
              className="px-3 py-1.5 rounded-xl bg-red-500 text-black font-bold text-xs uppercase hover:bg-red-400 transition flex-shrink-0"
            >
        ...
        {showEndConfirm && (
          ...
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="glass-panel p-6 sm:p-8 rounded-3xl max-w-md w-full border border-red-500/50 shadow-[0_0_50px_rgba(239,68,68,0.35)] flex flex-col gap-4 text-center"
            >
              <div className="w-14 h-14 rounded-full bg-red-950/80 border border-red-500/60 mx-auto flex items-center justify-center text-red-400">
                <LogOut className="w-7 h-7" />
              </div>

              <h3 className="text-xl font-bold text-red-300">
                Conclude Technical Assessment?
              </h3>

              <p className="text-xs text-gray-300 font-sans leading-relaxed">
                &ldquo;Are you sure you want to end the interview? You will not be able to continue after ending it.&rdquo;
              </p>
              ...
              <div className="flex items-center justify-between gap-3 pt-2">
                <button
                  ...
                  className="w-1/2 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-gray-300 hover:text-white transition text-xs"
                >
                  Cancel &amp; Continue
                </button>
                ...
                <button
                  ...
                  className="w-1/2 py-2.5 rounded-xl bg-red-600 text-white font-bold text-xs uppercase hover:bg-red-500 transition shadow-[0_0_15px_rgba(239,68,68,0.5)] flex items-center justify-center gap-1.5"
                >
                  {endingInterview ? "Evaluating..." : "Confirm & End"}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* ─── Top Control Bar: Active Phase, Timer, Integrity & End Interview ─── */}
      <div className="glass-panel p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 border border-cyan-500/30 shadow-[0_0_30px_rgba(0,240,255,0.15)]">
        ...
        {/* Center: Live Synchronized Countdown Timer & Integrity Score */}
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-950/90 border border-cyan-500/40 text-cyan-300 font-black tracking-widest text-sm shadow-[0_0_15px_rgba(0,240,255,0.2)]">
          ...
          <div
            ...
            title="Integrity score adjusted by tab switches and full-screen exits"
          >
            <ShieldAlert className="w-3.5 h-3.5" />
            <span>Integrity: {integrityScore}/10</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-red-950/80 border border-red-500/50 text-red-300 hover:bg-red-900/80 transition text-xs font-bold flex items-center gap-1.5 shadow-[0_0_15px_rgba(239,68,68,0.3)]"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          {/* ─── Turn History Accordion (With Strict 0-10 Scores & Rubrics) ─── */}
          {history.length > 0 && (
            <div className="glass-panel rounded-2xl border border-cyan-500/20 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span>Turn History &amp; Evaluator Audit ({history.length} Questions)</span>
                </div>
                        ...
                        >
                          <div className="flex items-center justify-between text-[10px] text-gray-500">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded font-black font-mono border ${
                                  score >= 8
                          ...
                          {h.evaluation && (
                            <div className="text-[11px] text-gray-300 font-mono bg-slate-900/60 p-2 rounded-lg border border-slate-800 flex flex-col gap-1">
                              <span className="text-cyan-400 font-bold">Evaluator Critique:</span>
                              <span className="leading-relaxed">{h.evaluation}</span>
                            </div>
                          )}
                        </div>
                      );
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="glass-panel p-4 rounded-2xl flex flex-col gap-3 border border-cyan-500/25 shadow-[0_4px_25px_rgba(0,0,0,0.4)]">
            ...
            <div className="pt-2 border-t border-cyan-500/10 text-[10px] text-gray-500 text-center">
              Strict 0–10 Evidence-Based Evaluator Active
            </div>
          </div>
        </div>
      </div>
